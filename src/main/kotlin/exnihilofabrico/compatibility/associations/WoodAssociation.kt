package exnihilofabrico.compatibility.associations
...
data class WoodTextures(var planks: Identifier?, var log: Identifier?, var leaves: Identifier?){
...
data class WoodBlocks(var planks: Identifier?, var slab: Identifier?, var log: Identifier?, var leaves: Identifier?){
...
fun ArtificeResourcePack.ClientResourcePackBuilder.addCrucibleModel(wood: WoodAssociation) {
    ...
    } ?: ExNihiloFabrico.LOGGER.warn("Failed to generate ${wood.crucible} model because log texture is undefined.")
}
...
fun ArtificeResourcePack.ClientResourcePackBuilder.addBarrelModel(wood: WoodAssociation) {
    ...
    } ?: ExNihiloFabrico.LOGGER.warn("Failed to generate ${wood.barrel} model because planks texture is undefined.")
}
...
fun ArtificeResourcePack.ClientResourcePackBuilder.addSieveModel(wood: WoodAssociation) {
    ...
    } ?: ExNihiloFabrico.LOGGER.warn("Failed to generate ${wood.sieve} model because planks texture is undefined.")
}
...
fun ShapedRecipeBuilder.addCrucibleRecipe(wood: WoodAssociation) {
    ...
    } ?: ExNihiloFabrico.LOGGER.warn("Failed to generate ${wood.barrel} recipe because log are undefined.")
}
    ...
    (wood.blocks.planks)?.let { planks ->
        (wood.blocks.slab)?.let { slab ->
            this.pattern(
                ...
                .result(wood.barrel, 1)
        } ?: ExNihiloFabrico.LOGGER.warn("Failed to generate ${wood.barrel} recipe because slab is undefined.")
    } ?: ExNihiloFabrico.LOGGER.warn("Failed to generate ${wood.barrel} recipe because planks are undefined.")
}
    ...
    (wood.blocks.planks)?.let { planks ->
        (wood.blocks.slab)?.let { slab ->
            this.pattern(
                ...
                .result(wood.sieve, 1)
        } ?: ExNihiloFabrico.LOGGER.warn("Failed to generate ${wood.sieve} recipe because slab is undefined.")
    } ?: ExNihiloFabrico.LOGGER.warn("Failed to generate ${wood.sieve} recipe because planks are undefined.")
}