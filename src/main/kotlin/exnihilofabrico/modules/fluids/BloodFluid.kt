...
class BloodFluid(isStill: Boolean): AbstractFluid(isStill, fluidSettings, Supplier { block }, Supplier { bucket }, Supplier { flowing }, Supplier { still }) {
    override fun matchesType(fluid: Fluid) = fluid == still || fluid == flowing
    companion object {
        val fluidSettings = FluidSettings("blood")
        val still = BloodFluid(true)
        val flowing = BloodFluid(false)
        val bucket = BucketItem(still, ModFluids.bucketItemSettings)
        val block = BaseFluidBlock(still, ModFluids.blockSettings)
        val tag = TagRegistry.fluid(Identifier("c:blood"))
    }
}